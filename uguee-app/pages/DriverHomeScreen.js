...
const DriverHomeScreen = ({ navigation }) => {
  ...
  return (
    ...
    onPress={() => setShowDatePicker(true)} // Al presionar, mostramos el picker
  >
    <Text style={styles.dateTimeInput}>
      {/* Muestra la fecha formateada o un placeholder */}
      {dateTime ? new Date(dateTime).toLocaleString() : 'Seleccionar fecha y hora'}
    </Text>
  </TouchableOpacity>
                  {showDatePicker && (
                <DateTimePicker
                    ...
                    // 1. Oculta el picker para que no se vuelva a abrir.
                    setShowDatePicker(false); 

                    // 2. Comprueba si el usuario seleccionó una fecha (y no presionó "cancelar").
                    if (event.type === 'set' && selectedDate) {
                        // 3. Oculta el teclado manualmente ANTES de actualizar el estado.
                        Keyboard.dismiss(); 
                        
                        // 4. Actualiza el estado con la nueva fecha.
